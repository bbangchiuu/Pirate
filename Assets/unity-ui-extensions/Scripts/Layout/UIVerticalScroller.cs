...
namespace UnityEngine.UI.Extensions
{
    ...
    public class UIVerticalScroller : MonoBehaviour
    {
        ...
        public void Update()
        {
            ...
            for (var i = 0; i < elementLength; i++)
            {
                var canvasGrp = _arrayOfElements[i].GetComponent<CanvasGroup>();
                if (canvasGrp) canvasGrp.interactable = false;
                if (minDistance == distance[i])
                {
                    minElementsNum = i;
                    if (canvasGrp) canvasGrp.interactable = true;
                    var childText = _arrayOfElements[i].GetComponentInChildren<Text>();
                    if (childText) result = childText.text;
                }
            }
