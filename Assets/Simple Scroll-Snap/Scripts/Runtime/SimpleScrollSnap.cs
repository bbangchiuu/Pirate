...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        public bool IsPanelLocked(int panelIdx)
        {
            return Panels[panelIdx].activeSelf == false;
        }
        ...
        private int DetermineNearestPanel()
        {
            ...
            if (minDistance == distances[currentPanel])
            {
                return currentPanel;
            }
        ...
        public void GoToPreviousPanel()
        {
            ...
            if (nearestPanel != 0)
            {
                int prePanel = nearestPanel - 1;
                if (prePanel < 0)
                {
                    prePanel = nearestPanel;
                }
                while (IsPanelLocked(prePanel))
                {
                    prePanel--;
                    if (prePanel < 0)
                    {
            ...
            else
            {
                if (infinitelyScroll)
                {
                    int prePanel = NumberOfPanels - 1;
                    while (IsPanelLocked(prePanel))
                    {
                        prePanel--;
                        if (prePanel < 0)
                        {
                            prePanel += NumberOfPanels;
                        }
                    }
                    GoToPanel(prePanel);
                }
