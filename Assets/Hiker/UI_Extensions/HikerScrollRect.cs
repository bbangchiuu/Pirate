...
namespace Hiker.UI
{
    [AddComponentMenu("UI/Hiker Scroll Rect", 37)]
    [SelectionBase]
    [ExecuteAlways]
    ...
    /// </remarks>
    public class HikerScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        /// <summary>
        ...
        [SerializeField]
        private bool m_CheckAllBoundContent = false;
        ...
        public void CenterOnRectTransform(RectTransform rectTrans)
        {
            CenterOnRectTransform(rectTrans, 0);
            CenterOnRectTransform(rectTrans, 1);
        }

        public void CenterOnRectTransform(RectTransform rectTran, int axis)
        {
            var normalizeVal = GetNormalizePosition(rectTran, axis);
            SetNormalizedPosition(normalizeVal, axis);
        }

        public float GetNormalizePosition(RectTransform rectTran, int axis)
        {
            EnsureLayoutHasRebuilt();
            UpdateBounds();
            // How much the content is larger than the view.
            float hiddenLength = m_ContentBounds.size[axis] - m_ViewBounds.size[axis];
            var contentBoundsMinPosition = m_ContentBounds.min[axis] + m_ViewBounds.extents[axis];
            var contentBoundsMaxPosition = m_ContentBounds.max[axis] - m_ViewBounds.extents[axis];

            var b = GetBoundByRectTransform(rectTran);
            var normalizeVal = (b.center[axis] - contentBoundsMinPosition) / (contentBoundsMaxPosition - contentBoundsMinPosition);
            return normalizeVal;
        ...
        {
            var bounds = GetBoundByRectTransform(m_Content, false);
            if (m_CheckAllBoundContent && m_Content)
            {
                for(int i = 0; i < m_Content.childCount; ++i)
                {
                    var child = m_Content.GetChild(i) as RectTransform;
                    bounds.Encapsulate(GetBoundByRectTransform(child, true));
                }
            }
            return bounds;
        }
        private Bounds GetBoundByRectTransform(RectTransform rectTrans, bool recursive = false)
        {
            if (rectTrans == null)
                return new Bounds();
            ...
            if (recursive)
            {
                ...
                {
                    var child = rectTrans.GetChild(i) as RectTransform;
                    bounds.Encapsulate(GetBoundByRectTransform(child));
                }
            }
