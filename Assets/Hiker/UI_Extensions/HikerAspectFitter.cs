...
namespace Hiker.UI
{
    ...
    [AddComponentMenu("Layout/Hiker/Hiker Aspect Ratio Fitter", 142)]
    [ExecuteAlways]
    ...
    /// </summary>
    public class HikerAspectFitter : UIBehaviour, ILayoutSelfController
    {
        ...
        [SerializeField] private ActiveCondition m_ActiveCondition = ActiveCondition.Always;
        /// <summary>
                ...
                case AspectMode.EnvelopeParent:
                    {
                        ...
                        var aspectX = parentSize.y * aspectRatio;
                        var fitRatio = aspectRatio;
                        if (m_ActiveCondition == ActiveCondition.LargerRatio)
                        {
                            if (aspectX > parentSize.x)
                            {
                                fitRatio = parentSize.x / parentSize.y;
                            }
                        }
                        else if (m_ActiveCondition == ActiveCondition.SmallerRatio)
                        {
                            if (aspectX < parentSize.x)
                            {
                                fitRatio = parentSize.x / parentSize.y;
                        ...
                        if ((parentSize.y * fitRatio < parentSize.x) ^ (m_AspectMode == AspectMode.FitInParent))
                        {
                            sizeDelta.y = GetSizeDeltaToProduceSize(parentSize.x / fitRatio, 1);
                        }
                        else
                        {
                            sizeDelta.x = GetSizeDeltaToProduceSize(parentSize.y * fitRatio, 0);
                        }
